...
int main(int argc, char** argv) {
  ...
  try {
    ...
    GifWriter gifWriter(outputFile);

    int noColors = 256;

    ColorMapGenerator *greyScaleColorMap = new GrayScaleColorMapGenerator(noColors);

    gifWriter.SetupGif(width, height, greyScaleColorMap);

    int loopCount = 0;
    int res = gifWriter.WriteNetscapeLoopExtension(loopCount); 
    if(res == GIF_ERROR) {
        ...
        if(avcodec_receive_frame(codecContext, frame) == 0) {
          if(counter >= startFrameIndex && counter < noFramesToExtract && (counter % 2) == 0) {
            if(!hasCopiedPrevFrame) {
              ...
              gifWriter.WriteSingleImage(width, height, frame->data[0]);
            }
            else {
    ...
    fprintf(stderr, "No of Colors can only be 256 or less\n");
  }
